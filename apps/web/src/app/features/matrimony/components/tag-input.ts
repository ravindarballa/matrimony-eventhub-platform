...
@Component({
  ...
  styles: `
    ...
    .box:focus-within { border-color: var(--brand); box-shadow: 0 0 0 1px var(--brand) inset; }
    .box.full { background: rgb(0 0 0 / 0.03); }
    .tag { display: inline-flex; align-items: center; gap: 0.3rem;
           background: #e6e5f5; color: var(--brand); border-radius: 999px;
    ...
    .tag button:hover { background: rgb(var(--brand-rgb) / 0.15); }
...
export class TagInput {
  ...
  /**
   * Per-entry length. A hobby is a word or two; an achievement is a sentence,
   * and capping it at a hobby's length would truncate the very specifics that
   * make it worth reading.
   */
  readonly maxLength = input(MAX_TAG_LENGTH);
  ...
  protected commit(): void {
    // The configured limit, not the hobby default - this is what actually
    // enforces it, since the maxlength attribute only guards typing.
